...
const PRODUCTS = [
    {
        ...
        flow: [
            ...
            { label: 'FastAPI validates Pydantic schema + checks auth', color: '#a855f7' },
            { label: 'AI task dispatched to LangGraph orchestrator', color: '#10b981' },
            { label: 'LangGraph calls OpenAI/Anthropic with retry logic', color: '#10b981' },
            { label: 'Token metering & usage metering updated', color: '#f59e0b' },
            { label: 'Results written to PostgreSQL + Redis cache', color: '#10b981' },
            { label: 'React UI updates in real-time via WebSocket', color: '#00e5ff' },
        ],
    },
    {
        id: 'interview-platform',
        title: 'Interview Intelligence Platform',
        badge: 'Client Product',
        badgeColor: '#f59e0b',
        ...
        flow: [
            ...
            { label: 'AWS API Gateway routes to FastAPI on Lambda', color: '#f59e0b' },
            { label: 'Live transcription stored in DynamoDB', color: '#f59e0b' },
            { label: 'AI analysis dispatched to serverless workers', color: '#a855f7' },
            { label: 'Session metrics persisted to PostgreSQL', color: '#10b981' },
            { label: 'React UI updates with live highlights', color: '#00e5ff' },
        ],
    },
    {
        id: 'enterprise-saas',
        ...
        flow: [
            { label: 'React UI triggers bulk data processing', color: '#00e5ff' },
            { label: 'FastAPI triggers Azure Data Factory pipeline', color: '#3b82f6' },
            { label: 'Databricks transformations executed on Spark', color: '#3b82f6' },
            { label: 'Heavy job offloaded to Celery workers', color: '#ef4444' },
            { label: 'Results synced to Blob Storage & PostgreSQL', color: '#10b981' },
            { label: 'Audit logs generated and stored in Azure', color: '#3b82f6' },
            { label: 'Dashboard refreshed with latest ETL data', color: '#00e5ff' },
        ],
    },
]
